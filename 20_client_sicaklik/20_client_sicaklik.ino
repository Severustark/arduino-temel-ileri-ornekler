// WEB sunucusuna client olarak bağlanıp, anasayfada sıcaklık verisini çeken program
...
const char *sunucu = "xxx.xxx.xxx.xxx";           // bağlanılacak olan web sayfasının adresi
...
void loop(void) 
{
  ...
  while (istemci.available())     // sunucudan gelen bilgi var mı?
  {
    ...
    if(okunan_veri.indexOf("Sicaklik")!= -1)            // okunan satırda "Sicaklik" yazısı var mı?
    {                                                   // evet varmış
      int baslangic=okunan_veri.indexOf("Sicaklik");    // "Sicaklik" yazısının başlangıç indexini al
      sicak_deger="";                                   // sicak_deger değişkenini bir temizle
      
      //.......sıcaklık değerini çekmek için I.yol.............
      for(int i=baslangic+8;i<baslangic+13;i++)
        sicak_deger+=okunan_veri[i];
        
      //.......sıcaklık değerini çekmek için II.yol.............
      //sicak_deger=okunan_veri.substring(baslangic+8,baslangic+13);
      
      Serial.print("Sicaklik degeri:");
      Serial.println( sicak_deger);
    }
    //----------------------------------------------------------------------
  }
  Serial.println("");
